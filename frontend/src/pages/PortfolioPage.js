...
import { Plus, Trash2, Wallet, TrendingUp, TrendingDown, BarChart3, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export default function PortfolioPage() {
  ...
  const addHolding = async () => {
    if (!form.asset_name || !form.quantity || !form.buy_price) { toast.error('Fill all fields'); return; }
    try {
      const res = await api.post('/portfolio/holdings', {
        ...form, asset_id: form.asset_name.toLowerCase().replace(/[^a-z0-9]/g, ''),
  ...
  const enrichedHoldings = holdings.map(h => {
    const live = allPrices.find(p => p.id === h.asset_id);
    const currentPrice = live?.price || h.buy_price;
    const invested = h.quantity * h.buy_price;
    const currentValue = h.quantity * currentPrice;
    const pnl = currentValue - invested;
    const pnlPct = invested > 0 ? (pnl / invested) * 100 : 0;
    return { ...h, currentPrice, currentValue, invested, pnl, pnlPct, isLive: !!live };
  });
  ...
  const pieData = enrichedHoldings.map((h, i) => ({
    name: h.asset_name, value: h.currentValue, color: COLORS[i % COLORS.length],
  }));
    ...
    <div className="space-y-6 page-enter" data-testid="portfolio-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">Track holdings with real-time P&L</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-white/5 border border-white/10">
            <div className={`live-dot ${connected ? '' : 'bg-red-500'}`} />
            <span className="text-[10px] font-data text-white/50">{connected ? 'LIVE P&L' : 'OFFLINE'}</span>
          </div>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="bg-[#09090B] border-white/10 text-white">
              <DialogHeader><DialogTitle className="text-white">Add New Holding</DialogTitle></DialogHeader>
              <div className="space-y-4 mt-4">
                ...
                <div>
                  ...
                  <Select value={form.asset_type} onValueChange={v => setForm({ ...form, asset_type: v })}>
                    <SelectTrigger className="bg-black/50 border-white/10 text-white text-sm mt-1" data-testid="holding-type-select"><SelectValue /></SelectTrigger>
                ...
                <Button className="w-full bg-[#6366F1] hover:bg-[#4F46E5] text-white active:scale-95" onClick={addHolding} data-testid="confirm-add-holding-btn">Add to Portfolio</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="glass-panel border-white/10" data-testid="total-invested-card">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#6366F1]/10 flex items-center justify-center"><Wallet className="w-5 h-5 text-[#6366F1]" /></div>
              <div>
                <p className="text-[10px] text-white/40 uppercase tracking-wider">Invested</p>
                <p className="text-lg font-bold text-white font-data">${totalInvested.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card className="glass-panel border-white/10" data-testid="current-value-card">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#00FF94]/10 flex items-center justify-center"><BarChart3 className="w-5 h-5 text-[#00FF94]" /></div>
              <div>
                <p className="text-[10px] text-white/40 uppercase tracking-wider">Current Value</p>
                <p className="text-lg font-bold text-white font-data">${totalValue.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card className={`glass-panel border-white/10 ${totalPnL >= 0 ? 'border-l-2 border-l-[#00FF94]' : 'border-l-2 border-l-[#FF2E2E]'}`} data-testid="total-pnl-card">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg ${totalPnL >= 0 ? 'bg-[#00FF94]/10' : 'bg-[#FF2E2E]/10'} flex items-center justify-center`}>
                {totalPnL >= 0 ? <ArrowUpRight className="w-5 h-5 text-[#00FF94]" /> : <ArrowDownRight className="w-5 h-5 text-[#FF2E2E]" />}
              </div>
              <div>
                <p className="text-[10px] text-white/40 uppercase tracking-wider">Total P&L</p>
                <p className={`text-lg font-bold font-data ${totalPnL >= 0 ? 'text-[#00FF94]' : 'text-[#FF2E2E]'}`}>
                  {totalPnL >= 0 ? '+' : ''}{totalPnL.toLocaleString(undefined, {minimumFractionDigits:2})}
                </p>
                <p className={`text-[10px] font-data ${totalPnL >= 0 ? 'text-[#00FF94]/70' : 'text-[#FF2E2E]/70'}`}>
        ...
        <Card className="glass-panel border-white/10" data-testid="holdings-count-card">
          <CardContent className="p-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#EAB308]/10 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-[#EAB308]" /></div>
              <div>
                <p className="text-[10px] text-white/40 uppercase tracking-wider">Holdings</p>
                <p className="text-lg font-bold text-white font-data">{holdings.length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="glass-panel border-white/10 overflow-hidden" data-testid="holdings-table-card">
            <CardHeader className="pb-2"><CardTitle className="text-sm text-white/80">Holdings</CardTitle></CardHeader>
                ...
                <table className="w-full">
                  <thead className="border-b border-white/5">
                    <tr className="text-[10px] text-white/40 uppercase tracking-wider">
                      ...
                      <th className="text-right py-3 px-5">Buy</th>
                  ...
                  <tbody>
                    {enrichedHoldings.map(h => (
                      <tr key={h.holding_id} className={`market-row border-b border-white/[0.03] ${priceChanges[h.asset_id] === 'up' ? 'price-flash-up' : priceChanges[h.asset_id] === 'down' ? 'price-flash-down' : ''}`} data-testid={`holding-row-${h.holding_id}`}>
                        <td className="py-3 px-5">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {h.isLive && <div className="live-dot" style={{width:4,height:4}} />}
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-5 text-right text-sm text-white font-data price-value">${h.currentPrice?.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td className={`py-3 px-5 text-right text-sm font-data font-medium ${h.pnl >= 0 ? 'text-[#00FF94]' : 'text-[#FF2E2E]'}`}>
                          <div>{h.pnl >= 0 ? '+' : ''}${Math.abs(h.pnl).toLocaleString(undefined, {minimumFractionDigits:2})}</div>
                          <div className="text-[10px] opacity-70">({h.pnlPct.toFixed(2)}%)</div>
                        </td>
                        <td className="py-3 px-5 text-right">
                          <Button variant="ghost" size="icon" className="w-7 h-7 text-white/30 hover:text-[#FF2E2E]"
                            onClick={() => deleteHolding(h.holding_id)} data-testid={`delete-holding-${h.holding_id}`}><Trash2 className="w-3.5 h-3.5" /></Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <Card className="glass-panel border-white/10" data-testid="allocation-chart-card">
          <CardHeader className="pb-2"><CardTitle className="text-sm text-white/80">Allocation</CardTitle></CardHeader>
          <CardContent>
            {pieData.length === 0 ? (
                ...
                <div className="space-y-2 mt-2">
                  {pieData.map((d, i) => (
                    <div key={i} className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-white font-data">{totalValue > 0 ? ((d.value / totalValue) * 100).toFixed(1) : 0}%</span>
                    </div>
                  ))}
                </div>
              </>
